...
TRANSPARENT = "#010101"
APP_TITLE = "EVE DPS"
APP_SUBTITLE = "v.1.3 by Leffe Brown"
...
def _generate_alert_wav(path):
    """Generate a soft two-tone alert wav (440 Hz + 880 Hz) at low amplitude."""
    sr = 44100
    dur = 0.4
    n = int(sr * dur)
    data = []
    for i in range(n):
        t = i / sr
        # Two tones, fade in/out envelope
...
if not os.path.exists(_DEFAULT_WAV_PATH):
    try:
        _generate_alert_wav(_DEFAULT_WAV_PATH)
    except Exception:
        _DEFAULT_WAV_PATH = None
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            ...
            if isinstance(loaded, dict):
                ...
                if isinstance(loaded.get("sort_mode"), str):
                    cfg["sort_mode"] = loaded["sort_mode"]
                if isinstance(loaded.get("manual_order"), list):
                    cfg["manual_order"] = [str(x) for x in loaded["manual_order"]]
                if isinstance(loaded.get("alarm"), dict):
                    cfg["alarm"].update(loaded["alarm"])
        except Exception:
            pass
    return cfg
...
def _scale_wav(src_path: str, volume: float) -> str:
    """Return path to a temporary WAV scaled to the given volume (0.0–1.0)."""
    volume = clamp(volume, 0.0, 1.0)
    if volume >= 0.99:
        return src_path   # no scaling needed
    try:
        ...
        if sw == 2:
            ...
            out_frames = struct.pack(f"<{n}h", *scaled)
        elif sw == 1:
            samples = list(struct.unpack(f"{n}B", frames))
            scaled  = [int((s - 128) * volume) + 128 for s in samples]
            out_frames = struct.pack(f"{n}B", *scaled)
        else:
            return src_path
...
class AlarmManager:
    def __init__(self, cfg_ref):
        ...
        self._alarming = {}      # char -> bool (currently in alarm state)
    ...
    def _acfg(self):
        return self.cfg_ref.get("alarm", {})

    def check(self, char, inc_dps):
        acfg = self._acfg()
        threshold = float(acfg.get("threshold", 500))
        triggered = inc_dps >= threshold

        with self._lock:
            was = self._alarming.get(char, False)
            self._alarming[char] = triggered

        # Visual blink state is toggled by a dedicated 1-second timer (see tick_blink)

        # Sound
        if acfg.get("enabled_sound", False):
            repeat = acfg.get("repeat", False)
            if repeat:
                if triggered and not self._is_repeat_running():
                    self._start_repeat()
                elif not triggered and self._is_repeat_running():
                    self._stop_repeat()
            else:
    ...
    def _play_once(self):
        ...
        if self._sound_playing: return   # prevent stacking
        def _run():
            ...
            try:
                ...
                if wav and os.path.exists(wav):
                    vol = float(acfg.get("volume", 0.7))
                    scaled = _scale_wav(wav, vol)
    ...
    def tick_blink(self):
        """Toggle blink state for all currently alarming chars. Call every ~1 second."""
        with self._lock:
            for char, alarming in self._alarming.items():
                if alarming:
                    self._blink_state[char] = not self._blink_state.get(char, False)
                else:
                    self._blink_state[char] = False
    ...
    def is_alarming(self, char):
        with self._lock:
            return self._alarming.get(char, False)

    def blink_state(self, char):
        with self._lock:
            return self._blink_state.get(char, False)

    def any_alarming(self):
        with self._lock:
            return any(self._alarming.values())
...
class DPSEngine:
    ...
    def _register_incoming(self, ts, dmg):
        if self.current_battle is None:
            self.current_battle = self._new_battle(ts)
    ...
    def _tick(self):
        ...
        for entry in _ENTRY_RE.split(chunk):
            ...
            if m:
                ...
                if ts:
                    try:
                        events.append((ts, "out", int(m.group(2)), extract_target(flat)))
                    except Exception: pass
                continue
            ...
            if m:
                ...
                if ts:
                    try:
                        events.append((ts, "in", int(m.group(2)), ""))
                    except Exception: pass
        ...
        with self._lock:
            for ts, kind, dmg, target in events:
                ...
                elif kind == "in": self._register_incoming(ts, dmg)
    ...
    def get_status(self):
        ...
        total_damage = total_secs = total_inc_dmg = total_inc_elapsed = 0.0
        for b in finished:
        ...
        return {
            ...
            "inc_tdps": (total_inc_dmg / total_inc_elapsed) if total_inc_elapsed > 0 else 0.0,
    ...
    def get_battle_records(self):
        with self._lock:
            ...
            if self.current_battle:
                ...
                records.append({
                    "start_ts": c["start_ts"], "end_ts": c["last_event_ts"],
                    "total_dmg": c["total_dmg"], "active_elapsed": c["active_elapsed"],
                    "dps": (c["total_dmg"] / c["active_elapsed"]) if c["active_elapsed"] > 0 else 0.0,
                    "hits": list(c["hits"]), "first_target": c["first_target"],
...
class AlarmSettingsPopup(tk.Toplevel):
    def __init__(self, master, app):
        ...
        # ── Threshold ──
        tk.Label(self, text="Inc.DPS threshold:", bg=C_BG, fg=C_MUTED,
        ...
        tk.Label(self, text="Volume:", bg=C_BG, fg=C_MUTED,
                 font=("Consolas", 9)).grid(row=3, column=0, sticky="e", **pad)
        self.var_vol = tk.DoubleVar(value=acfg.get("volume", 0.7))
        vol_frame = tk.Frame(self, bg=C_BG)
        vol_frame.grid(row=3, column=1, sticky="w", **pad)
        ...
        tk.Label(vol_frame, text="(system vol applies)", bg=C_BG, fg=C_MUTED,
                 font=("Consolas", 7)).pack(side="left", padx=4)

        # ── Repeat ──
        self.var_repeat = tk.BooleanVar(value=acfg.get("repeat", False))
        self.chk_repeat = tk.Checkbutton(self, text="Repeat while Inc.DPS exceeds threshold",
                                          variable=self.var_repeat,
                                          bg=C_BG, fg=C_TEXT, selectcolor=C_PANEL,
                                          activebackground=C_BG, activeforeground=C_HEADER,
                                          font=("Consolas", 9))
        self.chk_repeat.grid(row=4, column=0, columnspan=2, sticky="w", **pad)

        # ── WAV file ──
        tk.Label(self, text="Alert sound:", bg=C_BG, fg=C_MUTED,
                 font=("Consolas", 9)).grid(row=5, column=0, sticky="e", **pad)
        wav_frame = tk.Frame(self, bg=C_BG)
        wav_frame.grid(row=5, column=1, sticky="w", **pad)
    ...
    def _toggle_sound_opts(self):
        ...
        self.vol_slider.config(state=state)
        self.chk_repeat.config(state=state)

    def _browse_wav(self):
        path = filedialog.askopenfilename(
            title="Select alert WAV file",
            filetypes=[("WAV files", "*.wav"), ("All files", "*.*")])
        if path:
            self.var_wav.set(path)

    def _reset_wav(self):
        self.var_wav.set("(built-in)")

    def _test_sound(self):
        if not WINSOUND_OK:
            messagebox.showinfo("Sound", "winsound not available on this system.")
            return
        wav = self.var_wav.get()
        if wav == "(built-in)": wav = _DEFAULT_WAV_PATH
        if not wav or not os.path.exists(wav):
            messagebox.showwarning("Sound", "WAV file not found.")
            return
    ...
    def _save(self):
        ...
        # If sound was just disabled, stop any currently playing alarm
...
class GraphWindow(tk.Toplevel):
    ...
    def __init__(self, master, app):
        ...
        # Bind resize to the Toplevel BUT only act when on an edge
        ...
        # Thin drag bar at top (draggable area above notebook tabs)
        ...
        # Wrapper so we can overlay the ✕ button on the tab row
        ...
        # ✕ close button overlaid at top-right of notebook (same height as tabs)
        self.btn_close = tk.Label(nb_wrap, text="✕", bg=C_PANEL_2, fg=C_ICON,
                                  font=("Segoe UI Symbol", 10), padx=6, pady=4,
                                  cursor="hand2")
    ...
    def _on_mouse_move(self, e):
        ...
        try: self.config(cursor=self._cursor_for_edge(edge))
        except Exception: pass
    ...
    def _on_toplevel_press(self, e):
        self._on_press(e)

    def _on_toplevel_drag(self, e):
        self._on_drag(e)

    def _on_toplevel_release(self, e):
        self._on_release(e)
    ...
    def _on_resize(self, e):
        w = getattr(e, "width",  self._win_w)
        h = getattr(e, "height", self._win_h)
        ...
        # Resize grip indicator (small triangle at bottom-right)
        g = self.RESIZE_GRIP
        rx, ry = self._win_w - 2, self._win_h - 2
        self.bg_canvas.create_polygon(
            rx, ry-g, rx, ry, rx-g, ry,
            fill=C_BORDER, outline="")
        # No title bar — body fills the entire window
    ...
    def _get_joint_battles(self):
        ...
        for start_ts, end_ts, items in merged:
            ...
            for item in items:
                ...
                e = battle["chars"][char]
                e["hits"].extend(rec["hits"])
                e["total_dmg"] += rec["total_dmg"]
    ...
    def _history_total_width(self):
        return self.TIME_COL_W + self.ENEMY_COL_W + len(self.app.sorted_characters()) * 2 * self.STAT_COL_W
    ...
    def _build_history_tree(self):
        ...
        chars = self.app.sorted_characters()
        columns = ["time", "enemy"] + [f"{c}_dps" for c in chars for _ in (0,1)][::1]
        columns = ["time", "enemy"]
        ...
        # Scroll speed: 3 units per step for keyboard and mousewheel
        tree.bind("<Up>",       lambda e: (tree.yview_scroll(-3, "units"), "break"))
        tree.bind("<Down>",     lambda e: (tree.yview_scroll( 3, "units"), "break"))
    ...
    def _update_history_rows(self):
        ...
        for battle in battles:
            row = [f"{fmt_dt(battle['start_ts'])} ~ {fmt_dt(battle['end_ts'])}", battle.get("enemy", "")]
            for char in chars:
                e = battle["chars"][char]
...
class EVEUnifiedWindow(tk.Toplevel):
    ...
    def render_all(self, force_layout=False):
        ...
        x_alarm   = self._cur_w - 100
        x_sort    = self._cur_w - 115
        ...
        # Each button text has anchor="e": drawn LEFT of x. Center ≈ x - 8.
        # Use ±10px box around center to avoid overlap and ensure accurate hit.
        ...
        # ── Gather DPS values for bar graph ──
        statuses = {char: self.app.get_status(char) for char in chars}
        in_combat_any = any(statuses[c]["in_combat"] for c in chars)
        if in_combat_any:
            bar_values = {c: statuses[c]["dps"] for c in chars}
        else:
            bar_values = {c: statuses[c]["tdps"] for c in chars}
        ...
        for idx, char in enumerate(chars):
            ...
            self.cv.create_rectangle(6, y1, self._cur_w-6, y2,
                                     outline=C_BORDER, width=0.6, fill=row_fill)

            # DPS bar background
            if max_bar > 0:
                ratio = bar_values[char] / max_bar
                bar_w = int((self._cur_w - 12) * ratio)
                if in_combat_any:
                    bar_col = "#1a3a5a"   # blue tint, outgoing
                else:
                    bar_col = "#1a3520"   # green tint, cumulative
                self.cv.create_rectangle(6, y1, 6+bar_w, y2,
                                         fill=bar_col, outline="", stipple="gray50")
            ...
            if hover:
                # Three buttons evenly spaced at 14px intervals from name_r
                _BW = 14   # button spacing
                edit_x = cols["name_r"] - _BW        # ✎ leftmost
                hide_x = cols["name_r"] + _BW // 2   # ⊘ middle (7px right of name_r)
                ...
                edit_box = (edit_x-8, y1+2, edit_x+8, y2-2)
                hide_box = (hide_x-8, y1+2, hide_x+8, y2-2)
                if is_manual:
                    drag_x = cols["name_r"] + _BW * 2  # drag handle rightmost
                    for dy_off in [-4, 0, 4]:
                        self.cv.create_line(drag_x-6, yc+dy_off, drag_x+6, yc+dy_off,
                                            fill=C_ICON_H, width=1.5)
        ...
        visual_alarm_on = alarm_cfg.get("enabled_visual", True)
        ...
        if self.inc_expanded and chars:
            ...
            for idx, char in enumerate(chars):
                ...
                status = statuses[char]
                # Alarm check
                alarm_mgr.check(char, status["inc_dps"])
                alarming = visual_alarm_on and alarm_mgr.is_alarming(char)
                blink_on = alarm_mgr.blink_state(char)

                if alarming and blink_on:
                    row_bg = "#3a0a0a"
                    txt_fill = "#ffffff"
                elif alarming:
                    row_bg = "#3a0a0a"
                ...
                # Inc DPS bar background
                inc_max = max((statuses[c]["inc_dps"] for c in chars), default=0.0)
                if inc_max > 0:
                    inc_ratio = status["inc_dps"] / inc_max
                    inc_bar_w = int((self._cur_w-12) * inc_ratio)
                ...
                self.cv.create_text(cols["dps_r"]  - 4, ryc, anchor="e", text=fmt_num(status["inc_dps"]),   fill=txt_fill,  font=self.font_num_b)
    ...
    def _on_left_click(self, e):
        ...
        if header == "sort":
            sx = self.winfo_rootx() + e.x
            sy = self.winfo_rooty() + e.y + 10
            SortMenuPopup(self, self.app, sx, sy); return
        ...
        # Check drag handle first (manual sort)
        if info.get("drag_box"):
            x1, y1, x2, y2 = info["drag_box"]
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self._drag_char = char
                self._drag_y = e.y
                self.app.pause_updates = True
                return
        ...
        if info["edit_box"]:
            ...
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self._start_inline_edit(char); return
        if info["hide_box"]:
            x1, y1, x2, y2 = info["hide_box"]
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self.app.hide_character(char); return
    ...
    def _on_mouse_drag(self, e):
        ...
        for char, info in self.row_info.items():
            ...
            if y2 > last_y2:
                last_y2 = y2
                last_char = char
            if y1 <= e.y <= y2 and char != self._drag_char:
                found = char
                break
    ...
    def _apply_manual_reorder(self, moving_char, target_char):
        ...
        if target_char == "__END__":
            chars.append(moving_char)
        else:
            if target_char not in chars: return
            idx = chars.index(target_char)
            chars.insert(idx, moving_char)
        self.app.cfg["manual_order"] = chars
        save_config(self.app.cfg)
    ...
    def _on_motion(self, e):
        ...
        if header:
            tooltips = {
                "show_all": "Show all", "graph": "Recent battle",
                "refresh": "Refresh",   "min": "Minimize",
                "quit": "Quit",         "inc_toggle": "Toggle incoming",
        ...
        if not self.hover_char:
            self.tooltip.hide(); return
        info = self.row_info.get(self.hover_char)
        if not info: self.tooltip.hide(); return
        if info.get("edit_box"):
            x1, y1, x2, y2 = info["edit_box"]
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self.tooltip.show(e.x_root, e.y_root, "Set name"); return
        if info.get("hide_box"):
            x1, y1, x2, y2 = info["hide_box"]
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self.tooltip.show(e.x_root, e.y_root, "Hide"); return
        if info.get("drag_box"):
            x1, y1, x2, y2 = info["drag_box"]
            if x1 <= e.x <= x2 and y1 <= e.y <= y2:
                self.tooltip.show(e.x_root, e.y_root, "Drag to reorder"); return
...
class App(tk.Tk):
    ...
    def sorted_characters(self):
        ...
        if mode == SORT_NAME_DESC:
            return sorted(visible, key=natural_sort_key, reverse=True)
        ...
        if mode == SORT_MANUAL:
            ...
            # Put known chars in order, then append new ones alphabetically
